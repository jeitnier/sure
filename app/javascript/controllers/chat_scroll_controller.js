...
export default class extends Controller {
  ...
  // Messenger convention: sending YOUR OWN message always snaps to bottom and
  // re-pins, regardless of prior scroll/pin state. This is distinct from
  // handleAppend's pinned-only-follow rule, which still governs incoming/
  // broadcast appends (e.g. the assistant's reply streaming in later).
  ...
  storageKey() {
    return `chat-scroll:${this.chatIdValue}`;
  }
  ...
  writeState(state) {
    try {
      ...
      /* storage unavailable — degrade to always-pin */
    }
  }
}
